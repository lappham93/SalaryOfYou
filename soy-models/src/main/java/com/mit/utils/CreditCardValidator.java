...
public class CreditCardValidator // implements IValidator<String>
{
	...
	{		
		try
		{
			return isLengthAndPrefixCorrect(value);
		}
	...
	public boolean validateCode(final CreditCard creditCard,final String cardCode) {		
		int digits = 0;
		switch (creditCard) {
		case MASTERCARD:
		case VISA:
		...
		case DINERS_CLUB_US_AND_CANADA:
		case CHINA_UNIONPAY:
		case LASER:
		case MAESTRO:
		case SOLO:
		...
		Pattern pattern = Pattern.compile("^[0-9]{" + digits + "}$");
...
//	}
	...
	private CreditCard isDinersClubInternational(String creditCardNumber)
	{
		if (creditCardNumber.length() == 14 && 
				(creditCardNumber.startsWith("36") || creditCardNumber.startsWith("38") ||
						creditCardNumber.startsWith("39")))
		{
	...
	public static void main(String[] args) {
		String cardNumber = "38000000000006";
		String cvv = "222";
		CreditCardValidator test = new CreditCardValidator();
		CreditCard creditCard = test.determineCardId(cardNumber);
		System.out.println(creditCard.name());
		System.out.println(test.validateCode(creditCard, cvv));
	}
}
