...
public class BizFollowDAO extends CommonDAO {
	private final Logger _logger = LoggerFactory.getLogger(BizFollowDAO.class);
	private static Lock _lock = new ReentrantLock();
	private static BizFollowDAO _instance;

	private final String TABLE_NAME = "biz_follow";

	public static BizFollowDAO getInstance() {
		if(_instance == null) {
			_lock.lock();
			try {
				if(_instance == null) {
					_instance = new BizFollowDAO();
				}
			} finally {
				_lock.unlock();
			}
	...
	private BizFollowDAO() {}

	public long addFollow(int uid, int bizId) {
		long rs = MongoErrorCode.NOT_CONNECT;
		if(dbSource != null) {
			try {
				Document objFinder = new Document("_id", bizId).append("followIds", new Document("$ne", uid));
				Document update = new Document("$push", new Document("followIds", new Document("$each", Arrays.asList(uid))
					.append("$position", 0)));
				...
				if(rs > 0) {
					...
					objFinder = new Document("_id", bizId);
	...
	public long removeFollow(int uid, int bizId) {
		...
		if(dbSource != null) {
			try {
				Document objFinder = new Document("_id", bizId);
				Document update = new Document("$pull", new Document("followIds", uid));
	...
	public List<Integer> getUserFollowed(int bizId) {
	...
	public boolean isFollowed(int userId, int bizId) {
		...
		if(dbSource != null) {
			try {
				Document objFinder = new Document("_id", bizId).append("followIds", userId);
				Document projection = new Document("followIds", userId);
	...
	public List<Integer> getUserFollowed(int bizId, int from, int count) {
		List<Integer> listBiz = Collections.emptyList();
		if(dbSource != null) {
			try {
				Document objFinder = new Document("_id", bizId);
	...
	public int getTotalFollowed(int bizId) {
		int total = 0;
		if(dbSource != null) {
			try {
				Document objFinder = new Document("_id", bizId);
	...
	public Map<Integer, Integer> getTotalFollowedByListId(List<Integer> bizIds) {
		Map<Integer, Integer> total = new HashMap<Integer, Integer>();
		if(dbSource != null) {
			try {
				Document objFinder = new Document("_id", new Document("$in", bizIds));
				Document projection = new Document("total", 1);
