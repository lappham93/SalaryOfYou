 ...
 * @since Oct 10, 2016
 */
public class Model3DClient {
    private static final Class _ThisClass = Model3DClient.class;
	private static final Logger _Logger = LoggerFactory.getLogger(_ThisClass);
	private static final String _name = "model3d";
	private TClientPool.BizzConfig _bizzCfg;
    
    private static Model3DClient instance = new Model3DClient();
    
    private Model3DClient(){
        assert (_name != null && !_name.isEmpty());
		_initialize();
    }
    
    public static Model3DClient getInstance(){
        return instance;
    }

	private void _initialize() {
        System.out.println("Model3DClient._initialize");
	...
	public int ping() {
		...
		try {
            ...
            _Logger.error("Model3DClient.ping", ex);
    ...
    public int putMPhoto(TMPhoto tmp) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("Model3DClient.putMPhoto", ex);
			return MErrorDef.BAD_CONNECTION;
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("Model3DClient.putMPhoto", ex);
    ...
    public int multiPutMPhoto(List<TMPhoto> list) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("Model3DClient.multiPutMPhoto", ex);
			return MErrorDef.BAD_CONNECTION;
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("Model3DClient.multiPutMPhoto", ex);
    ...
    public void owPutMPhoto(TMPhoto tmp) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("Model3DClient.owPutMPhoto", ex);
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("Model3DClient.owPutMPhoto", ex);
		}
    ...
    public TMPhotoResult getMPhoto(long mvId) {
		...
		for (int retry = 0; retry < bCfg.getNRetry(); ++retry) {
			...
			try {
				...
				pool.invalidateClient(cli, ex);
                _Logger.error("Model3DClient.getMPhoto", ex);
				continue;
			} catch (TException ex) {
				pool.invalidateClient(cli, ex);
                _Logger.error("Model3DClient.getMPhoto", ex);
