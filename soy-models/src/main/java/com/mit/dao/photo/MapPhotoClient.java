 ...
 * @since Mar 21, 2016
 */
public class MapPhotoClient {
    private static final Class _ThisClass = MapPhotoClient.class;
	private static final Logger _Logger = LoggerFactory.getLogger(_ThisClass);
	private static final String _name = "mapphoto";
	private TClientPool.BizzConfig _bizzCfg;
    
    private static MapPhotoClient instance = new MapPhotoClient();
    
    private MapPhotoClient(){
        assert (_name != null && !_name.isEmpty());
		_initialize();
    }
    
    public static MapPhotoClient getInstance(){
        return instance;
    }
