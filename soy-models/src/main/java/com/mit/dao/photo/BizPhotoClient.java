 ...
 * @since Mar 21, 2016
 */
public class BizPhotoClient {
    private static final Class _ThisClass = BizPhotoClient.class;
	private static final Logger _Logger = LoggerFactory.getLogger(_ThisClass);
	private static final String _name = "bizphoto";
	private TClientPool.BizzConfig _bizzCfg;
    
    private static BizPhotoClient instance = new BizPhotoClient();
    
    private BizPhotoClient(){
        assert (_name != null && !_name.isEmpty());
		_initialize();
    }
    
    public static BizPhotoClient getInstance(){
        return instance;
    }
