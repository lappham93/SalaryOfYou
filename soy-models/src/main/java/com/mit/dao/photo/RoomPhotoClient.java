 ...
 * @since Mar 11, 2016
 */
public class RoomPhotoClient {
    private static final Class _ThisClass = RoomPhotoClient.class;
	private static final Logger _Logger = LoggerFactory.getLogger(_ThisClass);
	private static final String _name = "roomphoto";
	private TClientPool.BizzConfig _bizzCfg;
    
    private static RoomPhotoClient instance = new RoomPhotoClient();
    
    private RoomPhotoClient(){
        assert (_name != null && !_name.isEmpty());
		_initialize();
    }
    
    public static RoomPhotoClient getInstance(){
        return instance;
    }

	private void _initialize() {
        System.out.println("RoomPhotoClient._initialize");
	...
	public int ping() {
		...
		try {
            ...
            _Logger.error("RoomPhotoClient.ping", ex);
    ...
    public int putMPhoto(TMPhoto tmp) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("RoomPhotoClient.putMPhoto", ex);
			return MErrorDef.BAD_CONNECTION;
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("RoomPhotoClient.putMPhoto", ex);
    ...
    public int multiPutMPhoto(List<TMPhoto> list) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("RoomPhotoClient.multiPutMPhoto", ex);
			return MErrorDef.BAD_CONNECTION;
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("RoomPhotoClient.multiPutMPhoto", ex);
    ...
    public void owPutMPhoto(TMPhoto tmp) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("RoomPhotoClient.owPutMPhoto", ex);
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("RoomPhotoClient.owPutMPhoto", ex);
		}
    ...
    public TMPhotoResult getMPhoto(long mvId) {
		...
		for (int retry = 0; retry < bCfg.getNRetry(); ++retry) {
			...
			try {
				...
				pool.invalidateClient(cli, ex);
                _Logger.error("RoomPhotoClient.getMPhoto", ex);
				continue;
			} catch (TException ex) {
				pool.invalidateClient(cli, ex);
                _Logger.error("RoomPhotoClient.getMPhoto", ex);
