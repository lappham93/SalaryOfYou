 ...
 * @since Mar 3, 2016
 */
public class CommentPhotoClient {
    private static final Class _ThisClass = CommentPhotoClient.class;
	private static final Logger _Logger = LoggerFactory.getLogger(_ThisClass);
	private static final String _name = "commentphoto";
	private TClientPool.BizzConfig _bizzCfg;
    
    private static CommentPhotoClient instance = new CommentPhotoClient();
    
    private CommentPhotoClient(){
        assert (_name != null && !_name.isEmpty());
		_initialize();
    }
    
    public static CommentPhotoClient getInstance(){
        return instance;
    }

	private void _initialize() {
        System.out.println("CommentPhotoClient._initialize");
	...
	public int ping() {
		...
		try {
            ...
            _Logger.error("CommentPhotoClient.ping", ex);
    ...
    public int putMPhoto(TMPhoto tmp) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("CommentPhotoClient.putMPhoto", ex);
			return MErrorDef.BAD_CONNECTION;
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("CommentPhotoClient.putMPhoto", ex);
    ...
    public int multiPutMPhoto(List<TMPhoto> list) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("CommentPhotoClient.multiPutMPhoto", ex);
			return MErrorDef.BAD_CONNECTION;
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("CommentPhotoClient.multiPutMPhoto", ex);
    ...
    public void owPutMPhoto(TMPhoto tmp) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("CommentPhotoClient.owPutMPhoto", ex);
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("CommentPhotoClient.owPutMPhoto", ex);
		}
    ...
    public TMPhotoResult getMPhoto(long mvId) {
		...
		for (int retry = 0; retry < bCfg.getNRetry(); ++retry) {
			...
			try {
				...
				pool.invalidateClient(cli, ex);
                _Logger.error("CommentPhotoClient.getMPhoto", ex);
				continue;
			} catch (TException ex) {
				pool.invalidateClient(cli, ex);
                _Logger.error("CommentPhotoClient.getMPhoto", ex);
