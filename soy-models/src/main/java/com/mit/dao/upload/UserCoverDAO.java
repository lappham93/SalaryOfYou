...
public class UserCoverDAO extends CommonDAO {
	private final Logger _logger = LoggerFactory.getLogger(UserCoverDAO.class);
	private static Lock _lock = new ReentrantLock();
	private static UserCoverDAO _instance;

	private final String TABLE_NAME = "users_cover";

	public static UserCoverDAO getInstance() {
		if (_instance == null) {
			_lock.lock();
			try {
				if (_instance == null) {
					_instance = new UserCoverDAO();
				}
			} finally {
				_lock.unlock();
			}
...
//		}
	...
	public boolean isSameVersion(int id, int covVer) {
		Map<String, Object> data = null;
		if(dbSource != null) {
			try {
				Document objFinder = new Document("_id", id).append("covVer", covVer);
				Document projection = new Document("covVer", 1);
				Document doc = dbSource.getCollection(TABLE_NAME).find(objFinder).projection(projection).first();
				if(doc != null && !doc.isEmpty()) {
					return true;
				}
			} catch(final Exception e) {
				_logger.error("isSameVersion ", e);
			}
		}
	...
	public int updateData(int userId, int covVer, byte[] data) {
		int rs = MongoErrorCode.NOT_CONNECT;
		if(dbSource != null) {
			try {
				MongoMapper mapper = new MongoMapper();
				Document filter = new Document("_id", userId);
				Document obj = new Document("$set", new Document("data", data).append("covVer", covVer));
