 ...
 * @since Mar 21, 2016
 */
public class BoothPhotoClient {
    private static final Class _ThisClass = BoothPhotoClient.class;
	private static final Logger _Logger = LoggerFactory.getLogger(_ThisClass);
	private static final String _name = "boothphoto";
	private TClientPool.BizzConfig _bizzCfg;
    
    private static BoothPhotoClient instance = new BoothPhotoClient();
    
    private BoothPhotoClient(){
        assert (_name != null && !_name.isEmpty());
		_initialize();
    }
    
    public static BoothPhotoClient getInstance(){
        return instance;
    }

	private void _initialize() {
        System.out.println("BizPhotoClient._initialize");
	...
	public int ping() {
		...
		try {
            ...
            _Logger.error("BizPhotoClient.ping", ex);
    ...
    public int putMPhoto(TMPhoto tmp) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("BizPhotoClient.putMPhoto", ex);
			return MErrorDef.BAD_CONNECTION;
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("BizPhotoClient.putMPhoto", ex);
    ...
    public int multiPutMPhoto(List<TMPhoto> list) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("BizPhotoClient.multiPutMPhoto", ex);
			return MErrorDef.BAD_CONNECTION;
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("BizPhotoClient.multiPutMPhoto", ex);
    ...
    public void owPutMPhoto(TMPhoto tmp) {
		...
		try {
			...
			pool.invalidateClient(cli, ex);
            _Logger.error("BizPhotoClient.owPutMPhoto", ex);
		} catch (TException ex) {
			pool.invalidateClient(cli, ex);
			_Logger.error("BizPhotoClient.owPutMPhoto", ex);
		}
    ...
    public TMPhotoResult getMPhoto(long mvId) {
		...
		for (int retry = 0; retry < bCfg.getNRetry(); ++retry) {
			...
			try {
				...
				pool.invalidateClient(cli, ex);
                _Logger.error("BizPhotoClient.getMPhoto", ex);
				continue;
			} catch (TException ex) {
				pool.invalidateClient(cli, ex);
                _Logger.error("BizPhotoClient.getMPhoto", ex);
